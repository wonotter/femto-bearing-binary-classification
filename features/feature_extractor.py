...
def extract_time_domain_features(signal):
    ...
    return {
        ...
        'skewness': np.mean((signal - mean_val) / (std_val + 1e-10) ** 3),
        'peak_to_peak': np.max(signal) - np.min(signal),
    }

# STFT 가번 주파수 영역 특징 추출
def extract_frequency_domain_features(signal, fs=SAMPLING_FREQ):
    ...
    features['total_energy'] = np.mean(power)
    ...
    # 스펙트럼 첨도 - 특정 주파수에 에너지가 집중되어 있는지 계산
    mean_spectrum = np.mean(power, axis=1)
    mu = np.mean(mean_spectrum)
    std = np.std(mean_spectrum) + 1e-10
    features['spectral_spread'] = np.mean(((mean_spectrum - mu) / std) ** 4)
